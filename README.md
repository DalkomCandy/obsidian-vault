...
- **진단 패널**: 설정(⚙️) 맨 아래에서 지도 API·위치 권한·동기화·로컬 AI·저장된 데이터 상태를 한눈에 볼 수 있어요. 문제가 있는 항목은 원인과 해야 할 일까지 같이 알려주고, "복사" 버튼을 누르면 지금 실행 중인 빌드 번호까지 포함된 텍스트가 복사돼서 그대로 붙여넣어 물어보면 됩니다.
- **휴대폰 전용 레이아웃**: 화면이 좁으면(768px 미만) 지도가 화면 전체를 쓰고, 지역 선택·검색·설정·카테고리 필터는 지도 위에 떠 있는 바 두 줄로 옮겨가서 목록 시트는 여행 목록부터 바로 시작합니다. 시트 손잡이는 위아래로 끌면 원하는 높이로, 그냥 누르면 살짝 보기 → 반쯤 → 거의 전체 순서로 바뀌고, 목록에서 장소를 누르면 지도가 보이도록 자동으로 접혀요. 끌어서 순서/일차를 바꾸는 것도 터치로 됩니다.
